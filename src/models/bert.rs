...
use crate::layers::{Linear, LayerNorm, HiddenAct, CUBLASLT};
...
impl BertEmbeddings {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        if config.position_embedding_type != PositionEmbeddingType::Absolute {
            candle_core::bail!("Bert only supports absolute position embeddings");
        }
...
impl BertClassificationHead {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let n_classes = match &config.id2label {
            None => candle_core::bail!("`id2label` must be set for classifier models"),
...
impl BertModel {
    pub fn load(vb: VarBuilder, config: &Config, model_type: ModelType) -> Result<Self> {
        ...
        let (pool, classifier) = match model_type {
            ...
            ModelType::Classifier => {
                if config.model_type == Some("bert".to_string()) {
                    candle_core::bail!("`classifier` model type is not supported for Bert");
                }
                (
                    Pool::Cls,
        ...
        if pool != Pool::Mean && pool != Pool::Cls {
            candle_core::bail!("Pool type {pool:?} is not supported");
        }
...
impl Model for BertModel {
    ...
    fn predict(&self, batch: Batch) -> Result<Tensor> {
        match &self.classifier {
            None => candle_core::bail!("`predict` is not implemented for this model"),
            Some(classifier) => {
