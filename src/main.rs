...
use models::{BertModel, Config, RobertaConfig, RobertaModel};
...
impl Args {
    fn build_model_and_tokenizer(&self) -> Result<(RobertaModel, Tokenizer)> {
        ...
        // println!("tokenizer padding params: {:?}", tokenizer.get_padding());
...
fn rub_batch_tei() -> Result<()> {
    ...
    let revision = "main".to_string().to_owned();
    let use_pth = true;
